...
int model_allocate_triangle_buffer(Model *model, GLuint triangles)
{
    ...
    {
        log(ERROR,"model %s already has a triangle buffer");
        return -1;
    }
    if (!triangles)
    {
        log(ERROR,"cannot allocate 0 triangles!");
        return -1;
    }
    model->triangle_array = (GLuint *)malloc(sizeof(GLuint)*3*triangles);
    if (!model->triangle_array)
    {
        log(ERROR,"failed to allocate triangle buffer");
        return -1;
    }
...
int model_allocate_vertex_buffer(Model *model, GLuint vertices)
{
    ...
    {
        log(ERROR,"model %s already has a vertex buffer");
        return -1;
    }
    if (!vertices)
    {
        log(ERROR,"cannot allocate 0 vertices!");
        return -1;
    }
...
int model_allocate_attribute_buffer(Model *model, GLuint attributes)
{
    ...
    if (!attributes)
    {
        log(ERROR,"cannot allocate 0 attributes!");
        return -1;
    }
