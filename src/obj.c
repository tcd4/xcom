...
int obj_va_search(float *v,float *a,int count,float vtarget[3],float atarget[6])
{
    ...
    if (!v || !a)
    {
        log(ERROR,"error, missing a buffer to search");
        return -1;/*not found*/
    }
    for (i = 0; i < count;i++)
    {
...
int obj_file_convert_to_model(Obj *objFile,Model *model)
{
    ...
    {
        log(ERROR,"failed to alloacte vertex / attribute buffers!");
             ...
             )
{
    ...
    if (obj == NULL)
    {
        log(ERROR,"cannot draw obj, none specified!");
        return;
    }
    glPushMatrix();
